...
public class ImageFormatManagerTests
{
    ...
    {
        Assert.Equal(1, this.DefaultFormatsManager.ImageEncoders.Select(item => item.Value).OfType<PcxEncoder>().Count());
        ...
        Assert.Equal(1, this.DefaultFormatsManager.ImageDecoders.Select(item => item.Value).OfType<PcxDecoder>().Count());
