...
using SixLabors.ImageSharp.Formats.Dng;
...
public class ImageFormatManagerTests
{
    ...
    public void IfAutoLoadWellKnownFormatsIsTrueAllFormatsAreLoaded()
    {
        ...
        Assert.Equal(1, this.DefaultFormatsManager.ImageEncoders.Select(item => item.Value).OfType<DngEncoder>().Count());
        ...
        Assert.Equal(1, this.DefaultFormatsManager.ImageDecoders.Select(item => item.Value).OfType<DngDecoder>().Count());
